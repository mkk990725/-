...
function buildEvaluationPrompt(analysisInput, config) {
  ...
  return [
    ...
    "不要承诺精确比分，不要承诺稳定盈利。任何投注结构都只能表达概率分层与成本效率，不能承诺收益。比分和半全场只能标注为娱乐参考。",
    "必须先分析球队怎么踢、谁承担什么功能、双方强弱点如何对位。必须给三个比赛分支：常规分支、打穿分支、钝化/冷门分支，每个分支写触发条件、比分区间、方向和概率等级。盘口和赔率只能放在最后做验证。",
    "建议输出 JSON，字段至少包含 source_check、is_analyzable、analysis_score、confidence_score、match_filter、filter_reason、tactical_profile、player_functions、matchup、branches、market_check、first_half、full_time、key_evidence、information_gaps、source_reliability、uncertainty、team_data_check、option_layers、portfolio_structure、cost_efficiency、review_checklist、entertainment_top3。option_layers 必须包含 main_pick、secondary_pick、small_hedge、avoid。analysis_score 是 0-100 的可分析度/证据充足度评分，即使证据不足也要给出低分并说明原因，不能只写待补资料。",
    `分析技能：\n${skill}`,
